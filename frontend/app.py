...
if user_input:
    ...
    response = requests.post(f"{API_BASE}/chat", json={"message": user_input, "history": history_texts})
...
if st.session_state.proposed_time:
    ...
    if st.button("✅ Yes, book this meeting"):
        ...
        if booking.status_code == 200:
            ...
            st.session_state.messages.append({"role": "bot", "text": f"✅ Your meeting has been booked for {local_time.strftime('%B %d at %I:%M %p')}."})
            st.session_state.proposed_time = None
        else:
